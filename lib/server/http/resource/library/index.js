...
module.exports = async(function *(resolve, reject, url, modules, specs) {
    ...
    let module, file, extname, language, type;
    ...
    if (file) {
        ...
        if (type === 'code' && extname === '.js') {
            ...
            if (!module.code) {
                resolve(new Resource({'404': 'Module "' + module.ID + '" does not have a "code" script.'}));
                return;
            }
        ...
        else if (type === 'page' && extname === '.js') {

            if (!module.page) {
                resolve(new Resource({'404': 'Module "' + module.ID + '" does not have a "page" script.'}));
                return;
            }

            let output = yield module.page(language);
            ...
            if (!module.polymer) {
                resolve(new Resource({'404': 'Module "' + module.ID + '" does not implement a control.'}));
                return;
            }
        ...
        resolve(new Resource({
            '404': 'Module "' + module.ID +
            '" does not have a type "' + type + '" with a extension "' + extname + '".'
        }));
        return;
