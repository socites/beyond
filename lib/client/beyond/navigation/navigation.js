function Navigation(beyond, pages, events) {
    ...
    let navigateDefaultUrl = Delegate(this, function () {
    ...
    let navigateWebWarning = () => {
        ...
        if (window.location.search) pathname = `${this.pathname}${window.location.search}`;

        beyond.navigate(`/web_warning?next=${encodeURIComponent(pathname)}`);
        return true;
    ...
    let onpopstate = Delegate(this, () => this.pathname === '/' ? navigateDefaultUrl() : updateNavigation());
    ...
    this.navigate = function (pathname, state) {
        ...
        const event = {'event': 'redirect', 'cancellable': true, 'async': true};
        timerNavigate = setTimeout(() => events.trigger(event, this.pathname, response => {

            if (response && typeof response.pathname === 'string') {
                navigate(response.pathname, state);
                return;
            }
