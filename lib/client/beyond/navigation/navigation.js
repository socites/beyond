function Navigation(beyond, pages, events) {
    ...
    var updateNavigation = Delegate(this, function (callback) {
        ...
        if (navigationID === 1) {
            ...
            var webWarningConfig = beyond.params.webWarning;
            webWarningConfig = (typeof webWarningConfig !== 'boolean') ? !!webWarningConfig : webWarningConfig;
            var webWarningExcludes = (webWarningConfig) ? webWarningConfig.excludes : undefined;
            if (webWarningExcludes) {
                webWarningExcludes = webWarningExcludes.split(',');
            }
            else {
                webWarningExcludes = [];
            }

            if (webWarningExcludes.indexOf('/web_warning') === -1) {
                webWarningExcludes.push('/web_warning');
            }

            if (beyond.params.local && !!webWarningConfig &&
                webWarningExcludes.indexOf(this.pathname) === -1 && !!beyond.pages.routes['/welcome'] && !beyond.phonegap.isPhonegap) {
