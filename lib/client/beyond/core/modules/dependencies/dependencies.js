function Dependencies(module, dependencies) {
    ...
    this.set = function (value) {
        if (dependencies) {
            throw new Error('Module dependencies can only be set once');
        }
        dependencies = value;
        load();
    };
    ...
    function load() {
        ...
        let dependency;
        for (let bundle in dependencies.code) {
            mods.push(source(resource));
        }
        ...
        if (mods.length) {
            ...
            require(mods, function () {
                ...
                for (dependency in dependencies.code) {
                    ...
                    let name = dependencies.code[dependency];
                    modules[name] = args[i];
                    i++;
