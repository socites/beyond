 ...
 */
function Page(module, pathname, vdir, specs, Dependencies) {
    'use strict';
    ...
    Object.defineProperty(this, 'active', {'get': () => !!showTime});
    ...
    let showing;
    let showed; // Assure to call the show function only once
    let show = () => {
    ...
    let prepare = (callback) => {
        let timer = setTimeout(() => console.warn(`Page "${pathname}" is taking too much time to invoke the callback of the "prepare" function.`), 5000);
    ...
    let coordinate = new Coordinate('dependencies', 'react', function () {
        ...
        (typeof control.prepare === 'function') ? prepare(renderAndShow) : renderAndShow();
    ...
    this.initialise = function () {
        ...
        (module.react.loading) ? module.react.done(coordinate.react) : coordinate.done('react');
