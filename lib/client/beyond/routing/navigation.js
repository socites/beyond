function Navigation(beyond, pages, events) {
    ...
    let updateNavigation = (callback) => {
        ...
        showPage(this.pathname, search, history.state, () => {
            events.trigger('page-shown', this.pathname);
            callback();
        });

    };

    let onpopstate = () => (this.pathname === '/') ? navigateDefaultUrl() : updateNavigation();
