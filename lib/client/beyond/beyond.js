var Beyond = function (beyond) {
     ...
     * @returns {Promise<any>}
     */
    this.require = module => new Promise((resolve, reject) => {

        if (typeof module !== 'string') return reject(new Error('Invalid parameter'));
        let error = new Error(`Error loading or processing module "${module}"`);
