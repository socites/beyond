function Behavior(module, specs) {
    ...
    for (var name in specs.properties) {
        ...
        property.readOnly = (!!spec.readOnly || (!!spec.stateSource && !spec.observer));

        if (spec.stateSource) {
            property.notify = !!spec.notify;
        }
        else if (spec.notify) {
            throw new Error('Cannot specify the "notify" attribute as true, if the attribute "stateSource" is not set');
        }
