    ...
    if (specs.mode === 'phonegap') {
        ...
        for (let i in dirs) {
            ...
            for (let language of languages) {
                let path = require('path').join(pathJS, dirs[i], language);
                yield copy.recursive(dir, path);

                // Make phonegap build .zip file
                console.log(application.build.archive);
                // yield (require('./archive.js'))(application.name, path, application.build.archive);
            }
