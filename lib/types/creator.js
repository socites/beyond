...
module.exports = function (Compiler) {
    ...
    return function (module, config) {
        ...
        let compiler = new Compiler(module, config);
        ...
        var returnCode = async(function *(resolve, reject, language) {
            ...
            let output = require('./header')(module, compiler.extname);
            output += yield compiler.process(language);

            let resource = new Resource({'content': output, 'contentType': compiler.extname});
            resolve(resource);
        ...
        // Used by the compiler to set the specific configuration required on module.json
        this.setBuildConfig = async(function *(resolve, reject, json) {

            if (compiler.setBuildConfig) {
                yield compiler.setBuildConfig(json);
            }
