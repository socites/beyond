module.exports = function (application, libraries, config, runtime) {
    ...
    return function (language) {
        ...
        for (let i in config.imports.libraries) {
            ...
            if (version.hosts.ws && application.ws[library.name]) {
                hosts.libraries[library.name].ws = application.ws[library.name];
            }
        ...
        if (!runtime.local && application.build.libraries) {
            ...
            for (let name in hosts.libraries) {
                ...
                let host;
                host = application.build.libraries.host;
                if (host) {
                    host = require('url-join')(host, 'libraries/' + path);
                    host += '/';
                }
                else {
                    host = 'libraries/' + path + '/';
