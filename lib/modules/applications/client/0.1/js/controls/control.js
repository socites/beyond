function Control(ID, specs, state, events, controls) {
        ...
        require([moduleID], function (module) {
            ...
            if (typeof module === 'function') {
                ...
                if (!Controller) {
                    ...
                    error = 'Invalid control. Module "' + moduleID +
                        '" must expose an object with a "Control" attribute.';
                    done();
                    return;
                ...
                error = 'Module "' + moduleID + '" does not export an object';
                done();
                return;
            ...
            if (typeof controller.show !== 'function') {
                ...
                error = 'Controller of module "' + moduleID + '" does not specify a show method';
                done();
                return;
    ...
    var module = controls.modules.get(specs);
    if (module) {
    ...
    events.trigger({'event': 'routing', 'async': true}, pathname, state, function (response) {

        if (!response || !response.moduleID) {

            error = 'Pathname "' + pathname + '" does not have a module associated to it.';
            done();
            return;
        ...
        var module = controls.modules.get({'moduleID': moduleID});
        if (!module) {

            error = 'Module "' + moduleID + '" is invalid or does not have a registered control.';
            done();
            return;
