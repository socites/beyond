var Control = function (name, path) {
    ...
    if (path.substr(0, 7) !== 'http://' || path.substr(0, 8) !== 'https://') {
        ...
        if (path.substr(0, 12) === 'application/') {
            ...
            path = beyond.requireConfig.paths['application'] + '/' + componentPath + '/control.html';
            ...
            if (!libraryPath) {
                console.warn('library ' + libraryName + ' does not exist, check the control with path ' + original);
                return;
            }
            ...
            if (multilanguage && multilanguage.indexOf('polymer') !== -1) {
                path += '/control/' + beyond.params.language + '.html';
            }
            else {
                path += '/control.html';
            }
