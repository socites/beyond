function Dependencies(module, dependencies) {
    ...
    function getResourcePath(resource) {
        ...
        if (resource.substr(0, 12) !== 'application/' &&
            resource.substr(0, 10) !== 'libraries/') {
        ...
        var type = resource.pop();
        if (['control', 'page', 'code'].indexOf(type) === -1) {
            type = 'code';
            resource.push(type);
        }
        var moduleID = resource.join('/');

        if (moduleID.substr(0, 12) === 'application/') {
            return 'application/' + moduleID.substr(12) + '/' + type;
        }
        ...
        if (multilanguage && multilanguage.indexOf('code') !== -1) {
            return moduleID + '/' + type + '/' + beyond.params.language;
        }
        else {
            return moduleID + '/' + type;
        }
    ...
    function load() {
        ...
        for (var resource in dependencies.require) {
            ...
            var name = dependencies.require[resource];
            mods[resource] = getResourcePath(resource);
