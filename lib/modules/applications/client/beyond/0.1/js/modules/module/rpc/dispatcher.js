var Dispatcher = function (module, events) {
    ...
    var execute = function (channel, request, callback, options) {
        ...
        if (options.cache && options.cache.read) {
            ...
            if (cached) {
                ...
                if (item && typeof callback === 'function') {
                    ...
                    if (callback) callback(item.value, undefined, 'cache');
        ...
        channel.emit('rpc', request.serialized, function (id) {

            if (typeof id !== 'string' || !id) {
                if (callback) callback(undefined, 'Invalid acknowledge id.');
                return;
            }
    ...
    function processResponse(response) {
        ...
        if (typeof response !== 'object' || response === null || !response.id || !requests[response.id]) {
            console.log('RPC invalid response or invalid response id.', response);
            callback(undefined, 'Invalid response.');
            return;
        }

        var rq = requests[response.id];
        var callback = rq.callback;
        var options = rq.options;
        ...
        if (callback) callback(response.message, undefined, 'rpc');

    }

    if (module.library) module.library.bind('response', processResponse);
    else beyond.bind('response', processResponse);
