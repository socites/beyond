var Dispatcher = function (module, events) {
    ...
    function processResponse(response) {
        ...
        if (typeof response !== 'object' || response === null || !response.id) {
            console.log('RPC invalid response', response);
        ...
        if (response.error) {
            ...
            if (callbacks[response.id]) {
                ...
                callbacks[response.id](undefined, response.error);
        ...
        if (callbacks[response.id]) {
            callbacks[response.id](response, undefined, 'rpc');
            delete callbacks[response.id];
        }

    }

    module.library.bind('response', processResponse);
    beyond.bind('response', processResponse);
