...
function Pages(events) {
    ...
    this.register = function (module, _specs) {
        ...
        if (typeof _specs === 'string') {
            ...
            if (_specs.route.substr(0, 1) !== '/') _specs.route = '/' + route;
            route = _specs.route;
    ...
    var create = function (pathname, state, moduleID, initialState, parameter) {
    ...
    this.get = function (pathname, params, callback) {
        ...
        if (!moduleID) {
            ...
            // The ending part of the pathname specifies the parameter of the page
            // ex: /signin/token, the pathname is /signin and the parameter is the token
            while (dir = path.pop()) {
        ...
        if (moduleID) {
            callback(create(pathname, params, moduleID));
            return;
        }
        ...
        events.trigger(event, pathname, function (response) {
            ...
            callback(create(pathname, state, moduleID, initialState));
