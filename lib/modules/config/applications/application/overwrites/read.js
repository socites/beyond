...
module.exports = async(function *(resolve, reject, application, file) {
    ...
    if (!fs.existsSync(file) || !fs.statSync(file).isFile()) {
        ...
        output.error('invalid css values on application "'+application.name+'" error, file not found: '+file);
        resolve();
        return;
    ...
    try {
        ...
        output.error('css values "'+file+'" are invalid on application "'+application.name+'"');
