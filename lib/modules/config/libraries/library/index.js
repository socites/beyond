...
module.exports = function (paths, name, config, specs) {
    ...
    this.initialise = async(function *(resolve, reject) {
        ...
        if (typeof config !== 'object') {
            ...
            yaol.error(yaolMessenger,'invalid configuration of library "'+name+'" \n\t\t'+config);
        ...
        if (service) {
            ...
            if (!fs.existsSync(service.path) || !fs.statSync(service.path).isDirectory()) {
                ...
                yaol.error(yaolMessenger,'service directory "'+service.path+'" specified on library "'+this.name+'" does not exist'),
            ...
            if (service.config) {
                ...
                if (!fs.existsSync(service.config) || !fs.statSync(service.config).isFile()) {
                    ...
                    yaol.error(yaolMessenger,'service configuration file "'+path+'" specified on library "'+this.name+'" does not exist');
