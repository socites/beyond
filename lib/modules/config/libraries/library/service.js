 ...
 */
module.exports = function (library, service, serverServicesConfig) {
    ...
    if (!service.path) {
        throw new Error(`Service path not specified on library "${library}"`);
    }
    ...
    if (!fs.existsSync(service.path) || !fs.statSync(service.path).isDirectory()) {
        ...
        let message = `Service directory "${service.path}" ` +
            `specified on library "${library}" does not exist.`.red;
    ...
    let serverConfig = serverServicesConfig[library];
