...
module.exports = function (config, runtime) {
    ...
    this.initialise = async(function* (resolve, reject) {
        ...
        // Register the bundles library
        let bundles = require('path').join(__dirname, '../../types/client/library.json');
        yield config.libraries.register('bundles', bundles);
        ...
        // Register the polymer client library
        let polymer = require('path').join(__dirname, '../client/polymer/library.json');
        yield config.libraries.register('polymer', polymer);
