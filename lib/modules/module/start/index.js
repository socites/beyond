...
module.exports = function (module, config) {
    ...
    return async(function *(resolve, reject, language, overwrites) {
        ...
        if (config.start) {
            multilanguage = config.start.multilanguage;
            multilanguage = (typeof multilanguage === 'undefined') ? true : !!multilanguage;
            config.start.multilanguage = multilanguage;
        }

        if (multilanguage && !language) {

            let message = 'Language must be set on start script '.red +
