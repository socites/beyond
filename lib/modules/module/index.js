...
module.exports = function (config, specs, runtime) {
    ...
    this.initialise = async(function *(resolve, reject) {
        ...
        function setMultilanguageDefault(type) {

            if (typeof type.multilanguage === 'undefined') {
                type.multilanguage = true;
            }
            else {
                type.multilanguage = !!type.multilanguage;
            }

        }

        if (config.code) {
            setMultilanguageDefault(config.code);
            this.code = require('./code.js')(this, config.code);
        }
        if (config.page) {
            setMultilanguageDefault(config.page);
            this.page = require('./page.js')(this, config.page);
        }
        if (config.custom) {
            setMultilanguageDefault(config.custom);
            this.custom = require('./custom.js')(this, config.custom);
        }
        if (config.polymer) {
            setMultilanguageDefault(config.polymer);
            this.polymer = require('./polymer.js')(this, config.polymer);
        }
