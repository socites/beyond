function Behavior(module, specs) {
    ...
    function onDependenciesReady() {
        ...
        let dependencies = module.dependencies.modules;

        let creator = (typeof specs.creator === 'function') ? specs.creator() : undefined;
        let sna = new SNA(this, specs, creator, specs.properties, dependencies);

        // Check for already set properties
        let properties = sna.properties;
        properties.silence = true;
        for (let name in specs.properties) {
        ...
        let ReactDOM = dependencies.ReactDOM;
    ...
    this._setSNA = function (value) {
        ...
        let properties = this._sna.properties;
        properties.updateControl();

    };

    let setObserver = Delegate(this, function (name, property, observer) {

        let method = '_set' + name.substr(0, 1).toUpperCase() + name.substr(1) + 'Changed';
    ...
    for (let name in specs.properties) {
        ...
        let spec = specs.properties[name];
        let property = {};
    ...
    for (let index in specs.methods) {

        let method = specs.methods[index];
